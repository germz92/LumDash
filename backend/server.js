...
app.post('/api/tts/:tableId', authenticate, async (req, res) => {
  try {
    const { text } = req.body;
    const tableId = req.params.tableId;
    
    console.log('🎙️ TTS Request received:', { text: text?.substring(0, 50) + '...', tableId });
    ...
    if (!openai) {
      return res.status(503).json({ 
        error: 'AI TTS feature is not available. Please configure OpenAI API key.' 
      });
    }

    // Verify table access (same as chat endpoint)
    const table = await Table.findById(tableId);
    if (!table) {
      return res.status(404).json({ error: 'Table not found' });
    }
    ...
    const cleanText = text.replace(/[📸📅👥📷💾🗺️✈️✅]/g, '').replace(/\*\*/g, '').trim();
    
    if (!cleanText) {
      return res.status(400).json({ error: 'No valid text to convert' });
    }

    // Generate speech using OpenAI TTS
    const mp3 = await openai.audio.speech.create({
      model: "tts-1-hd", // Use tts-1-hd for highest quality
      voice: "nova", // Most natural, warm, casual female voice
      input: cleanText,
      response_format: "mp3",
    ...
    console.log('✅ TTS Success - Generated audio buffer:', buffer.length, 'bytes');
