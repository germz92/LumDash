...
// Serve SPA shell and root from the project root folder
app.use(express.static(path.join(__dirname, '..')));

// Serve static frontend assets from root subfolders
...
app.get('/api/gear-inventory', authenticate, async (req, res) => {
  try {
    const gear = await GearInventory.find().populate('notes.createdBy', 'fullName');
    res.json(gear);
  } catch (err) {
...
app.post('/api/gear-inventory/:id/notes', authenticate, async (req, res) => {
  try {
    ...
    // Store note on the inventory document that owns this serial (same model/label)
    let targetGear = gear;
    ...
    const allWithLabel = await GearInventory.find({ label: gear.label }).populate('notes.createdBy', 'fullName');
...
app.get('/api/reimbursements', authenticate, async (req, res) => {
  try {
    const requests = await ReimbursementRequest.find({ userId: req.user.id }).sort({ createdAt: -1 });
    res.json(requests);
  } catch (err) {
    console.error('Error fetching reimbursements:', err);
    res.status(500).json({ error: 'Failed to fetch reimbursement requests' });
  }
});
...
// Get single reimbursement request
app.get('/api/reimbursements/:id', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch reimbursement request' });
  }
});

// Update reimbursement request (save items + submit)
app.put('/api/reimbursements/:id', authenticate, async (req, res) => {
  try {
    const request = await ReimbursementRequest.findOne({ _id: req.params.id, userId: req.user.id });
    if (!request) return res.status(404).json({ error: 'Request not found' });
    ...
    res.status(500).json({ error: 'Failed to update reimbursement request' });
  }
});

// Delete reimbursement request (only drafts)
app.delete('/api/reimbursements/:id', authenticate, async (req, res) => {
  try {
    ...
    await ReimbursementRequest.findByIdAndDelete(req.params.id);
    res.json({ message: 'Reimbursement request deleted' });
  } catch (err) {
    console.error('Error deleting reimbursement:', err);
    res.status(500).json({ error: 'Failed to delete reimbursement request' });
  }
});
